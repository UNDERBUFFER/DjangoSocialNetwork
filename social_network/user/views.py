...
class Record(ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        self.queryset = models.Record.objects.filter(
            author_id=kwargs['author_id'])
        response = super().get(request, *args, **kwargs)
        response.data.append(
            {'Authorization': True if kwargs['author_id'] == request.user.id else False})
        return response
...
class Photo(ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        self.queryset = models.Photo.objects.filter(
            author_id=kwargs['author_id'])
        response = super().get(request, *args, **kwargs)
        data = []
        for i in response.data:
            data.append({'photo': i['photo'].replace(
    ...
    def post(self, request, *args, **kwargs):
        if kwargs['author_id'] == request.user.id:
            photo = models.Photo.objects.create(
                photo=request.data['photo'], author=request.user).photo.name
